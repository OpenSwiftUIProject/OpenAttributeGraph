...
#include <OpenAttributeGraphCxx/Vector/realloc_vector.hpp>
...
void *_Nullable OAG::details::realloc_vector(void* ptr, T& size, T new_size) {
    if (new_size == 0) {
        size = 0;
        free(ptr);
        return nullptr;
        ...
        if (size == msize / Alignment) {
            ...
            if (new_ptr == nullptr) {
                OAG::precondition_failure("memory allocation failed");
            } else {
...
template <typename T, size_t Alignment>
void *_Nullable OAG::details::realloc_vector(void* src, void* dst, T dstSize, T& size, T newSize) {
    if (newSize < dstSize) {
        if (src == dst) {
            return src;
        } else {
