...
// TODO:
// 1. select will both fail on AG and OAG with different error.
...
OAG::DebugServer::DebugServer(OAGDebugServerMode mode) {
    sockfd = -1;
    ip = 0;
    port = 0;
    token = arc4random();
    source = nullptr;
    connections = OAG::vector<std::unique_ptr<Connection>, 0, u_long>();

    // Create socket
    sockfd = socket(AF_INET, SOCK_STREAM, 0);
    if (sockfd < 0) {
        perror("OAGDebugServer: socket");
        return;
    }
    ...
    addr.sin_addr.s_addr = (mode & OAGDebugServerModeNetworkInterface) ? INADDR_ANY : htonl(INADDR_LOOPBACK);

    // Bind socket
    if (bind(sockfd, (const sockaddr *)&addr, sizeof(addr))) {
        perror("OAGDebugServer: bind");
        shutdown();
        return;
    }
    ...
    if (getsockname(sockfd, (sockaddr *)&addr, &slen)) {
        perror("OAGDebugServer: getsockname");
        shutdown();
        return;
    }
    ...
    if (mode & OAGDebugServerModeNetworkInterface) {
...
OAG::DebugServer::DebugServer(DebugServer&& other) OAG_NOEXCEPT
    : sockfd(other.sockfd)
    , ip(other.ip)
    , port(other.port)
    , token(other.token)
...
OAG::DebugServer& OAG::DebugServer::operator=(DebugServer&& other) OAG_NOEXCEPT {
    if (this != &other) {
        shutdown();
...
CFURLRef _Nullable OAG::DebugServer::copy_url() const {
    if (sockfd < 0) {
        return nullptr;
    }
    uint32_t converted_ip = htonl(ip);
...
void OAG::DebugServer::run(int timeout) const {
...
OAG::DebugServer* _Nullable OAG::DebugServer::start(OAGDebugServerMode mode) {
    if (
    ...
    return OAG::DebugServer::_shared_server;
}

void OAG::DebugServer::stop() {
...
OAG::DebugServer::Connection::Connection(DebugServer *s,int d) {
    server = s;
    sockfd = d;
    source = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, sockfd, 0, dispatch_get_main_queue());
    dispatch_set_context(source, this);
...
OAG::DebugServer::Connection::~Connection() {
    dispatch_source_set_event_handler_f(source, nullptr);
    dispatch_set_context(source, nullptr);
    close(sockfd);
}

void OAG::DebugServer::Connection::handler(void *_Nullable context) {
    Connection *connection = (Connection *)context;
    char buf[16] = {};
    if (blocking_read(connection->sockfd, &buf, 16)) {
        OAGDebugServerMessageHeader *header = (OAGDebugServerMessageHeader *)buf;
        header->unknown2 = 0;
...
CFDataRef _Nullable OAG::DebugServer::receive(Connection *, OAGDebugServerMessageHeader &, CFDataRef data) {
    @autoreleasepool {
        ...
        if (object && [object isKindOfClass:NSDictionary.class]) {
            ...
            if ([command isEqual:@"graph/description"]) {
                ...
                mutableDic[(__bridge NSString *)OAGDescriptionFormat] = (__bridge NSString *)OAGDescriptionFormatDictionary;
                CFTypeRef description = OAG::Graph::description(nullptr, mutableDic);
                if (description) {
...
void OAG::DebugServer::close_connection(OAG::DebugServer::Connection *target) {
...
OAG::DebugServer* _Nullable OAG::DebugServer::_shared_server = nullptr;
