...
table::table() {
    ...
    OAGGraphVMRegionBaseAddress = region;
    if (region == MAP_FAILED) {
        OAG::precondition_failure("memory allocation failure (%u bytes, %u)", _region_capacity, errno);
    }
    ...
    if (_malloc_zone == nullptr) {
        ...
        _malloc_zone = zone;
        malloc_set_zone_name(zone, "OpenAttributeGraph graph data");
    }
    #else
...
void table::make_pages_reusable(uint32_t page_index, bool reusable) OAG_NOEXCEPT {
    ...
    static bool unmap_reusable = []() -> bool {
        char *result = getenv("OAG_UNMAP_REUSABLE");
        if (result) {
...
// TO BE AUDITED
ptr<page> table::alloc_page(zone *zone, uint32_t needed_size) OAG_NOEXCEPT {
    lock();
...
// TO BE AUDITED
void table::dealloc_page_locked(ptr<page> page) OAG_NOEXCEPT {
...
OAG_CONSTEXPR
uint64_t table::raw_page_seed(ptr<page> page) const OAG_NOEXCEPT {
    assert_valid(page);
    lock();
...
void table::print() const OAG_NOEXCEPT {
    lock();
