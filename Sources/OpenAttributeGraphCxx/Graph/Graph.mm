...
void OAG::Graph::write_to_file(const Graph * _Nullable graph, const char * _Nullable name, uint8_t options) OAG_NOEXCEPT {
    @autoreleasepool {
        NSDictionary <NSString *, id> *options_dict = @{
            (__bridge NSString *)OAGDescriptionFormat: (__bridge NSString *)OAGDescriptionFormatDictionary,
            (__bridge NSString *)OAGDescriptionIncludeValues: @((options & 1) == 0),
            (__bridge NSString *)OAGDescriptionAllGraphs: @(graph == nil)
        };
